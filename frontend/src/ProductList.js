...
const REPAIRS_API_URL = 'https://fixmymaclk-2kww.onrender.com/api/productsRepair';
const PAYMENTS_API_URL = 'https://fixmymaclk-2kww.onrender.com/api/payments';
const PAYMENTS_API_URLwithItemcode = 'https://fixmymaclk-2kww.onrender.com/api/payments/with-itemcodes';
...
const ProductList = ({ darkMode }) => {
  ...
  const fetchProducts = () => {
    setLoading(true);
    setRefreshing(true);
    let url = `https://fixmymaclk-2kww.onrender.com/api/product-uploads`;
    fetch(url)
      .then((response) => {
        if (!response.ok) {
  ...
  const loadSupplierQuantities = async () => {
    ...
    try {
      const res = await fetch('https://fixmymaclk-2kww.onrender.com/api/suppliers');
  ...
  const handleUploadExcel = async () => {
    ...
    try {
      ...
      const response = await fetch('https://fixmymaclk-2kww.onrender.com/api/product-uploads/bulk-upload',
      {
        method: 'POST',
  ...
  const fetchUploadedRecords = async (page = 1) => {
    ...
    setUploadedError('');
    try {
      const res = await fetch(`https://fixmymaclk-2kww.onrender.com/api/product-uploads?page=${page}&limit=20`);
  ...
  const handleExportAllUploaded = async () => {
    try {
      const res = await fetch('https://fixmymaclk-2kww.onrender.com/api/product-uploads/export-all');
