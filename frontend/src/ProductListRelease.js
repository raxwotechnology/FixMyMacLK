...
//const API_URL = 'https://fixmymaclk.onrender.com/api/products';
 const API_URL = 'https://fixmymaclk.onrender.com/api/products';
const CLICKED_PRODUCTS_API_URL = 'https://fixmymaclk.onrender.com/api/clicked-products';

const REPAIRS_API_URL = 'https://fixmymaclk.onrender.com/api/productsRepair';
const PAYMENTS_API_URL = 'https://fixmymaclk.onrender.com/api/payments';
const PAYMENTS_API_URLwithItemcode = 'https://fixmymaclk.onrender.com/api/payments/with-itemcodes';
...
const ProductList = ({ darkMode }) => {
  ...
  const fetchProducts = () => {
    setLoading(true);
    setRefreshing(true);
    let url = `https://fixmymaclk.onrender.com/api/product-uploads`;
    fetch(url)
      .then((response) => {
        if (!response.ok) {
  ...
  const loadSupplierQuantities = async () => {
    ...
    try {
      const res = await fetch('https://fixmymaclk.onrender.com/api/suppliers');
  ...
  const handleUploadExcel = async () => {
    ...
    try {
      ...
      const response = await fetch('https://fixmymaclk.onrender.com/api/product-uploads/bulk-upload',
      {
        method: 'POST',
  ...
  const fetchUploadedRecords = async (page = 1) => {
    ...
    setUploadedError('');
    try {
      const res = await fetch(`https://fixmymaclk.onrender.com/api/product-uploads?page=${page}&limit=20`);
  ...
  const handleExportAllUploaded = async () => {
    try {
      const res = await fetch('https://fixmymaclk.onrender.com/api/product-uploads/export-all');
